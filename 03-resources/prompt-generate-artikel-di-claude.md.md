...
## Deliverables per Artikel
Setiap artikel harus disertai:
1. **File artikel** dalam format Markdown (`.md`)
2. **Judul** dan **short description** (maksimal 160 karakter)
3. **Caption untuk tiga platform**:
   - **LinkedIn**: informatif dan profesional, boleh lebih panjang
   - **Instagram**: dengan mention URL qadrlabs.com
...
- Menyebutkan library atau tool eksternal secara negatif jika konteks artikel tidak memerlukannya (fokus ke topik utama).
...
- If Pest is needed, include install step: `composer remove phpunit/phpunit`, `composer require pestphp/pest --dev --with-all-dependencies`, `./vendor/bin/pest --init`
...
install project dan pest bisa seperti contoh berikut ini
...
- Structure: `<body class="bg-gray-100 text-gray-800 font-sans p-6">` with inner `<div class="max-w-[2xl|7xl] mx-auto bg-white p-6 md:p-8 rounded-lg shadow-md">`
...
## Output Format
Tampilkan file markdown untuk artikel tersebut.

After the file is presented, provide:
- The title
- The short description with character count
-  category and tags article

Do NOT produce social media captions until I ask for them separately.
