---
title: "Tutorial CRUD Laravel 12 Untuk Pemula"
slug: "tutorial-crud-laravel-12-untuk-pemula"
category: "Laravel"
date: "2025-02-26"
status: "published"
---

[Laravel 12](https://qadrlabs.com/post/laravel-12) telah resmi dirilis pada 24 Februari 2025, membawa berbagai pembaruan dan peningkatan yang signifikan untuk ekosistem pengembangan web PHP. Meskipun difokuskan sebagai "maintenance release" dengan perubahan minimal yang tidak merusak sistem yang ada, Laravel 12 tetap menawarkan starter kits inovatif dan dukungan jangka panjang hingga Februari 2027.

Pada artikel-artikel sebelumnya, kita telah membahas fitur-fitur baru di Laravel 12 dan panduan lengkap [upgrade dari Laravel 11 ke Laravel 12](https://qadrlabs.com/post/upgrade-laravel-11-ke-laravel-12-panduan-lengkap-dengan-testing-pest). Melanjutkan rangkaian tutorial tersebut, dalam **tutorial Laravel 12** kali ini, kita akan membangun aplikasi manajemen produk dengan fitur **CRUD** (Create, Read, Update, Delete) secara langkah demi langkah.

Operasi CRUD merupakan fondasi dari hampir semua aplikasi web modern, dan menguasainya adalah keterampilan esensial bagi setiap developer. Tutorial ini dirancang untuk pemula yang baru memulai perjalanan dengan Laravel maupun developer berpengalaman yang ingin memperbarui pengetahuan mereka ke versi terbaru framework yang berjalan di PHP 8.2 atau lebih tinggi.
...
## Step 1: Buat Project Laravel {#step-1-buat-project-laravel}
Buka terminal, lalu run command berikut ini untuk membuat project laravel baru.
```
composer create-project laravel/laravel:^12.0 sample-app
```
Output:
```
$ composer create-project laravel/laravel:^12.0 sample-app
Creating a "laravel/laravel:^12.0" project at "./sample-app"
...
Kita tunggu sampai proses create project menggunakan composer selesai. 

Setelah selesai, kita pindah dulu ke direktori project.
```
cd sample-app
```
Lalu kita buka project kita di code editor. Di sini saya buka direktori project menggunakan visual studio code dengan run command berikut.
```
code .
```
Setelah kita run command di atas, direktori project akan terbuka di visual studio code.
...
Pada step ini kita buat file model dan migration sesuai dengan studi kasus, yaitu model dan migration untuk data product. Untuk membuat file model dan migration, buka terminal lalu run command berikut ini.
```
...
```
Save kembali  file `app/Models/Product.php`.

Pada baris kode di atas, kita atur supaya kolom `code`, `name`, `price` dan `stock` dapat diisi secara massal melalui input pengguna atau data yang dikirimkan dari form.

Selanjutnya kita sesuaikan file migration dengan studi kasus tutorial ini. Untuk menyesuaikan dengan studi kasus, buka file migration `database/migrations/xxxx_xx_xx_xxxxxx_create_products_table.php`, lalu kita tambahkan field `code`, `name`, `price` dan `stock` untuk table `products`.
...
Pada step ini kita run migration command untuk menambahkan table `products` di database `db_belajar_laravel`. Apabila teman-teman belum membuat file `db_belajar_laravel`, kita bisa membuat langsung ketika kita run migration command.

Sekarang kita buka kembali terminal, lalu run migration command.
```
php artisan migrate
```
...
```
Setelah selesai, kita bisa lihat ada database dan juga table ketika kita cek menggunakan tools seperti phpmyadmin ataupun run command sql untuk cek database.

## Step 5: Coding Fitur View Daftar Product {#step-5-coding-fitur-view-daftar-product}
Persiapan setup sudah selesai, sekarang kita masuk ke coding fitur yang pertama, yaitu fitur view daftar product. Untuk membuat fitur pertama, kita buat controler untuk menangani proses manage product.
```
...
Selanjutnya kita buat file untuk menampilkan daftar product, yaitu view `resources/views/products/index.blade.php`, lalu tambahkan baris kode berikut ini.
```
<!DOCTYPE html>
<html lang="en">
...
Save kembali  `resources/views/products/index.blade.php`.

Setelah coding controller dan view selesai, kita tambahkan route baru di file `routes/web.php`

```
Route::resource('product', \App\Http\Controllers\ProductController::class);
```

Untuk mengecek route yang sudah ditambahkan kita bisa run command berikut ini.
...
## Step 6: Coding Fitur Create Data{#step-6-coding-fitur-create-data}
Pada step ini kita akan menambahkan fitur untuk menambahkan data product. Kita buka kembali `app/Http/Controllers/ProductController.php`, lalu kita modifikasi method `create()`.
...
```
Seperti yang terlihat pada baris kode di atas, file view untuk menampilkan form create data product adalah `resources/views/products/create.blade.php`. Buat file `resources/views/products/create.blade.php`, lalu kita coding baris kode berikut ini.
...
Save kembali file `resources/views/products/create.blade.php`.

Proses tambah data pada form di arahkan ke route `product.store` yang diarahkan ke method `store()`. Buka kembali `app/Http/Controllers/ProductController.php`, lalu kita modifikasi method `store()`.
...
## Step 7: Coding Fitur View Satu Data{#step-7-coding-fitur-view-satu-data}
...
## Step 8: Coding Fitur Update Data{#step-8-coding-fitur-update-data}
...
Save kembali file `resources/views/products/edit.blade.php`.

Pada baris kode di atas, proses update data diarahkan ke route `post.update` yang mengarah ke method `update()`. Sekarang kita lengkapi method `update()` yang menangani proses update data product.
...
## Step 9: Coding Fitur Delete data{#step-9-coding-fitur-delete-data}
Sekarang kita coding fitur terakhir dari operasi CRUD, yaitu delete data product. Untuk menambahkan fitur delete data, kita buka kembali file controller. Pada class `ProductController`, kita modifikasi method `destroy()` yang menangani proses delete data.
...
![Akses project di browser](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/1-akses-halaman-daftar-product.png)

### Menambahkan Data Produk Baru

1. Klik tombol "Add Product" di bagian kanan atas halaman
	 ![akses halaman tambah data](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/2-akses-halaman-form-add-product.png)
2. Isi form dengan data produk contoh:
   - Name: Laptop ASUS ROG
   - Code: LP001
   - Price: 15000000
   - Stock: 10
   ![uji coba input data](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/3-uji-coba-tambah-data.png)
3. Klik tombol "Save" untuk menyimpan data
4. Sistem akan menampilkan halaman daftar produk dengan notifikasi "Product created successfully"
	 ![notifikasi data berhasil ditambahkan](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/4-data-berhasil-ditambahkan.png)

### Melihat Detail Produk

1. Pada halaman daftar produk, klik tombol "View" pada produk yang ingin dilihat
2. Sistem akan menampilkan halaman detail produk dengan informasi lengkap
	![akses halaman detail produk](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/5-view-detail-product.png)

### Mengedit Data Produk

1. Dari halaman daftar produk, klik tombol "Edit" pada produk yang ingin diubah
2. Sistem akan menampilkan form edit dengan data produk yang telah ada
3. Ubah data yang diinginkan, misalnya:
   - Name: Laptop ASUS ROG Strix
   - Price: 16500000
   - Stock: 8
   ![uji coba edit data](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/6-uji-coba-edit-data.png)
4. Klik tombol "Save Changes" untuk menyimpan perubahan
5. Sistem akan menampilkan halaman daftar produk dengan notifikasi "Product updated successfully"
	 ![notifikasi data berhasil diupdate](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/7-data-berhasil-diupdate.png)

### Menghapus Data Produk

1. Dari halaman daftar produk, klik tombol "Delete" pada produk yang ingin dihapus
2. Sistem akan menampilkan konfirmasi "Apakah Anda Yakin?"
	![tampil alert konfirmasi untuk delete data](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/8-uji-coba-delete-data.png)
3. Klik "OK" untuk mengkonfirmasi penghapusan
4. Sistem akan menampilkan halaman daftar produk dengan notifikasi "Product deleted successfully"
	![tampil notifikasi data berhasil dihapus](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/9-data-berhasil-didelete.png)

### Validasi Input

Anda juga dapat menguji validasi input dengan mencoba:
1. Menambahkan produk dengan kode yang sudah ada (untuk menguji validasi unique pada code)
	![uji coba validasi kode produk](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/10-uji-coba-validasi.png)
2. Mengirimkan form dengan field kosong (untuk menguji validasi required)
	![uji coba klik save saat form kosong](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/11-uji-coba-input-form-kosong.png)
...
![tampil paginasi saat data lebih dari 10](https://cdn.jsdelivr.net/gh/gungunpriatna/tes-repositori@master/laravel/crud-laravel-12/12-tampilan-pagination.png)
...
1. **Persiapan Awal**: Kita memulai dengan membuat project Laravel baru dan mengonfigurasi database untuk aplikasi.

2. **Pemodelan Data**: Kita membuat model Product beserta migrasi database yang diperlukan untuk menyimpan informasi seperti kode, nama, harga, dan stok produk.

3. **Implementasi Controller**: Kita mengembangkan ProductController dengan metode resource untuk menangani seluruh operasi CRUD.

4. **Pembuatan View**: Kita membuat beberapa halaman view menggunakan Blade template engine dan Tailwind CSS untuk antarmuka yang responsif dan modern.

5. **Pengembangan Fitur CRUD**:
