...
## Selanjutnya - Lesson 12 {#next-up}

Sistem autentikasi untuk Catatku kini telah selesai. `Auth::attempt($credentials)` menangani pencarian pengguna dan verifikasi password dalam satu pemanggilan, mengembalikan `true` jika berhasil dan `false` jika gagal, dan login yang berhasil diikuti dengan **regenerasi session** (`$request->session()->regenerate()`) untuk mencegah serangan session fixation dengan mengganti ID session. Pesan error login **dibuat samar secara sengaja** ("The email or password you entered is incorrect") agar penyerang tidak dapat mengetahui alamat email mana yang terdaftar, dan `back()->withErrors([...])->onlyInput('email')` mengarahkan kembali ke form sambil hanya mempertahankan nilai email, tanpa pernah mengirim password kembali ke browser. Logout menggunakan **route POST** (bukan GET, untuk mencegah serangan cross-site logout) dan menjalankan **logout tiga langkah** yang sangat penting: `Auth::logout()` menghapus identitas pengguna, `session()->invalidate()` menghancurkan seluruh data session, dan `session()->regenerateToken()` membuat token CSRF baru.

Di sisi routing, `->name('login')` diperlukan karena middleware `auth` Laravel mencari route dengan nama tersebut saat mengarahkan pengguna yang belum terautentikasi, `/entries` kini berada di dalam grup `middleware('auth')` sehingga daftar entri sepenuhnya privat, dan middleware `guest` mencegah pengguna yang sudah login mengakses halaman registrasi dan login. Dengan autentikasi yang sudah berjalan, method `index()` beralih dari `Entry::with('user')->latest()->get()` ke **`auth()->user()->entries()->latest()->get()`**: pada Lesson 6 hingga 9 memanggil `auth()->user()` akan membuat crash karena auth belum ada, tetapi kini query dibatasi dengan aman sehingga setiap pengguna hanya melihat entri mereka sendiri. Terakhir, helper `{{ url('/path') }}` menghubungkan tombol navigasi pada halaman utama.

Di Lesson 12, lesson terakhir, kita tidak akan menambahkan fitur baru apa pun. Sebagai gantinya, kita akan mundur sejenak dan melihat keseluruhan perjalanan: apa yang telah kita bangun, pola apa yang muncul berulang kali, dan ke mana Anda dapat melangkah selanjutnya untuk terus berkembang sebagai developer Laravel.
